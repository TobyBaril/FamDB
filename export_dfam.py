...
"""
    ...
    (curated/uncurated) × (consensus/hmm):
...
sys.path.append("/home/rhubley/projects/Dfam/Schemata/ORMs/python")
import dfamorm as dfam

# Import our Libs
sys.path.append(os.path.join(os.path.dirname(__file__), "../Lib"))
sys.path.append("/home/rhubley/projects/Dfam/Lib")
...
def run_chunk_worker(db_conn_str, family_ids, is_hmm, pickle_path, label, batch_size):
    ...
    try:
        ...
        LOGGER.info("%s: Fetching %d families", label, len(family_ids))
...
def _metadata_field_sizes(family):
    ...
    Size estimates:
      ...
      list  → 8 bytes × element count (covers clades: list of ints)
...
def build_component_from_pickles(
    ...
    LOGGER.info("Building %s from %d chunk(s)", label, len(chunk_files))
...
def main():
        ...
        for comp in comp_list:
            ...
            for part_num, family_ids in sorted(family_partitions[comp].items()):
                ...
                for chunk_idx, chunk_ids in enumerate(chunks):
                    ...
                    jobs.append((db_conn_str, chunk_ids, is_hmm_flag, pkl, label))
        return jobs
    ...
    futures = {}
    try:
        for db_conn, fids, is_hmm, pkl, label in all_chunk_jobs:
    ...
    try:
        for comp, part_num, chunks, nodes_for_part in assembly_jobs:
            ...
            assembly_futures[future] = f"{comp}.{part_num}"

        for future in as_completed(assembly_futures):
            label = assembly_futures[future]
            try:
                future.result()
            except Exception:
    ...
    elapsed = time.perf_counter() - _main_start
    LOGGER.info(
        "Export complete — total wallclock time: %s",
        str(datetime.timedelta(seconds=int(elapsed))),
    )
